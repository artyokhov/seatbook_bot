...
async def start_command_handler(message: Message) -> None:
    ...
    if user:
        ...
        await utils.safely_replace_message(
            ...
            new_text=messages.start_message_w_name.format(name=name),
        ...
        if not free_users:
            await utils.safely_replace_message(
                message,
                new_message_type=utils.MessageContentType.TEXT,
                new_text=messages.error_no_free_names_text,
        ...
        await utils.safely_replace_message(
            message,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.new_user_name_selection_text,
...
async def handle_register_query(query: CallbackQuery) -> None:
    ...
    await utils.safely_replace_message(
        query,
        new_message_type=utils.MessageContentType.TEXT,
        new_text=messages.preregister_verification_text.format(full_name=full_name),
        new_reply_markup=keyboards.confirm_registration_markup(user_id),
    )
...
async def handle_confirm_reg_query(query: CallbackQuery) -> None:
    ...
    name = (
        user.full_name.split()[1] if len(user.full_name.split()) > 1 else user.full_name
    )
    keyboard = (
        keyboards.start_markup_admin
...
async def handle_choose_booking_date_query(query: CallbackQuery) -> None:
    ...
    if not available_dates:
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.no_free_dates_text,
            new_reply_markup=keyboards.to_start_markup,
        )
    else:
        ...
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.choose_date_text,
            new_reply_markup=selection_keyboard,
        )
...
async def handle_seats_on_date_query(query: CallbackQuery) -> None:
    ...
    if book_date not in [date["timestamp"] for date in available_dates]:
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.no_seats_text,
            new_reply_markup=keyboards.no_seats_markup(book_date=book_date),
        )
        return
    ...
    await utils.safely_replace_message(
        query,
        new_message_type=utils.MessageContentType.MEDIA,
        new_text=messages.choose_seat_text,
        new_reply_markup=selection_keyboard,
    )
...
async def handle_book_date_seat_query(query: CallbackQuery) -> None:
    ...
    try:
        ...
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.MEDIA,
            new_text=messages.succesfull_booking_text.format(
                booking_date=booking_date, seat=booking.seat_number, full_name=full_name
            ),
            new_reply_markup=keyboards.succesfull_booking_markup,
        )
        ...
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.seat_is_occupied_text,
            new_reply_markup=keyboards.seat_is_occupied_markup(book_date),
        )
...
async def handle_manage_my_bookings_query(query: CallbackQuery) -> None:
    ...
    if users_bookings:
        bookings_text = messages.user_bookings_text_header.format(full_name=full_name)
        ...
        keyboard = keyboards.manage_my_bookings_markup
    else:
        bookings_text = messages.user_no_bookings_text
        keyboard = keyboards.to_start_markup

    await utils.safely_replace_message(
        query,
        new_message_type=utils.MessageContentType.MEDIA,
        new_text=bookings_text,
        new_reply_markup=keyboard,
    )
...
async def handle_booking_id_delete_query(query: CallbackQuery) -> None:
    ...
    await utils.safely_replace_message(
        query,
        new_message_type=utils.MessageContentType.TEXT,
        new_text=messages.succesfull_booking_delete_text,
        new_reply_markup=keyboards.to_start_markup,
    )
...
async def handle_see_colleagues_bookings_query(query: CallbackQuery) -> None:
    ...
    if not dates_with_visitors:
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.no_visitors_at_all_text,
            new_reply_markup=keyboards.to_start_markup,
        )
    else:
        ...
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.see_colleagues_bookings_choose_date_text,
            new_reply_markup=selection_keyboard,
        )
...
async def handle_see_colleagues_on_query(query: CallbackQuery) -> None:
    ...
    if visitors:
        ...
        for visitor in visitors:
            ...
            bookings_text += messages.see_colleagues_bookings_on_date_text_item.format(
    ...
    await utils.safely_replace_message(
        ...
        new_message_type=utils.MessageContentType.MEDIA,
        new_text=bookings_text,
        new_reply_markup=keyboards.see_colleagues_on_markup,
    )
...
async def handle_guest_seats_on_query(query: CallbackQuery) -> None:
    ...
    if book_date not in [date["timestamp"] for date in available_dates]:
        ...
        await utils.safely_replace_message(
            query,
            new_message_type=utils.MessageContentType.TEXT,
            new_text=messages.no_guest_seats_text,
            new_reply_markup=keyboards.no_guest_seats_markup(book_date=book_date),
        )
        return
...
async def handle_guest_date_seat_query(query: CallbackQuery) -> None:
    ...
    await utils.safely_replace_message(
        query,
        new_message_type=utils.MessageContentType.TEXT,
        new_text=messages.enter_full_name_text,
        new_reply_markup=keyboards.enter_guest_full_name_markup(
...
async def handle_write_guest_name_query(query: CallbackQuery) -> None:
    ...
    await utils.safely_replace_message(
        query,
        new_message_type=utils.MessageContentType.TEXT,
        new_text=messages.name_forcereply_text.format(
            book_date=book_date,
...
async def handle_guest_full_name_input(message: Message) -> None:
    ...
    try:
        ...
        logger.info(
            "%s создал бронирование: дата: %s, место: %s, тип: %s, для: %s",
            user_data.full_name,
            book_date,
        ...
        await utils.safely_replace_message(
            ...
            new_reply_markup=keyboards.succesfull_guest_booking_markup,
        )
        if message.reply_to_message:
            await utils.safely_delete_message(
                message.chat.id, message.reply_to_message.message_id
